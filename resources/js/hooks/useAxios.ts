...
export interface IAxiosForm<TForm extends FormDataType, TResponse = any, TError = any> {
  ...
  setError(errors: Partial<Record<keyof TForm | string, string>>): void;
  hasErrors: boolean;
  processing: boolean;
...
export function useAxios<TForm extends FormDataType, TResponse extends object, TError extends object = object>(initialValues?: TForm): IAxiosForm<TForm> {
  ...
  const setError: IAxiosForm<TForm>['setError'] = (fieldOrErrors, error = null) => {
    if(typeof fieldOrErrors == "object")
      setErrors(fieldOrErrors);
    else
      setErrors((prev) => ({...prev, [fieldOrErrors]: error}))
  }
